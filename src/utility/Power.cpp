...
static bool getI2CReg(uint8_t *result, uint8_t address, uint8_t *reg) {
  return (T4.I2C.readByte(address, *reg, result));
}

static bool setI2CReg(uint8_t address, uint8_t reg, uint8_t value) {
  return (T4.I2C.writeByte(address, reg, value));
}

bool POWER::setPowerBoostOnOff(bool en) {
  uint8_t data;
  if (T4.I2C.readByte(IP5306_ADDR, IP5306_REG_SYS_CTL1, &data) == true) {
    return T4.I2C.writeByte(IP5306_ADDR, IP5306_REG_SYS_CTL1, en ? (data | BOOST_SET_BIT) : (data & (~BOOST_SET_BIT)));
  }
  return false;
}
...
bool POWER::setPowerBoostSet(bool en) {
  uint8_t data;
  if (T4.I2C.readByte(IP5306_ADDR, IP5306_REG_SYS_CTL1, &data) == true) {
    return T4.I2C.writeByte(IP5306_ADDR, IP5306_REG_SYS_CTL1, en ? (data | SHORT_BOOST_BIT) : (data & (~SHORT_BOOST_BIT)));
  }
  return false;
}
...
bool POWER::setPowerVin(bool en) {
  uint8_t data;
  if (T4.I2C.readByte(IP5306_ADDR, IP5306_REG_SYS_CTL1, &data) == true) {
    return T4.I2C.writeByte(IP5306_ADDR, IP5306_REG_SYS_CTL1, en ? (data | VIN_ENABLE_BIT) : (data & (~VIN_ENABLE_BIT)));
  }
  return false;
}
...
bool POWER::setPowerWLEDSet(bool en) {
  uint8_t data;
  if (T4.I2C.readByte(IP5306_ADDR, IP5306_REG_SYS_CTL1, &data) == true) {
      return T4.I2C.writeByte(IP5306_ADDR, IP5306_REG_SYS_CTL1, en ? (data | WLED_SET_BIT) : (data & (~WLED_SET_BIT)));
  }
  return false;
}
...
bool POWER::setPowerBtnEn(bool en) {
  uint8_t data;
  if (T4.I2C.readByte(IP5306_ADDR, IP5306_REG_SYS_CTL0, &data) == true) {
    return T4.I2C.writeByte(IP5306_ADDR, IP5306_REG_SYS_CTL0, en ? (data | BOOST_BUTTON_EN_BIT) : (data & (~BOOST_BUTTON_EN_BIT)));
  }
  return false;
}
...
bool POWER::setLowPowerShutdownTime(ShutdownTime time)
{
  uint8_t data;
  bool ret;
  if (T4.I2C.readByte(IP5306_ADDR, IP5306_REG_SYS_CTL2, &data) == true){
    switch (time){
      case ShutdownTime::SHUTDOWN_8S:
        ret = T4.I2C.writeByte(IP5306_ADDR, IP5306_REG_SYS_CTL2, ((data & (~SHUTDOWNTIME_MASK)) | SHUTDOWNTIME_8S));
        break;
      case ShutdownTime::SHUTDOWN_16S:
        ret = T4.I2C.writeByte(IP5306_ADDR, IP5306_REG_SYS_CTL2, ((data & (~SHUTDOWNTIME_MASK)) | SHUTDOWNTIME_16S));
        break;
      case ShutdownTime::SHUTDOWN_32S:
        ret = T4.I2C.writeByte(IP5306_ADDR, IP5306_REG_SYS_CTL2, ((data & (~SHUTDOWNTIME_MASK)) | SHUTDOWNTIME_32S));
        break;
      case ShutdownTime::SHUTDOWN_64S:
        ret = T4.I2C.writeByte(IP5306_ADDR, IP5306_REG_SYS_CTL2, ((data & (~SHUTDOWNTIME_MASK)) | SHUTDOWNTIME_64S));
        break;
      default:
        ret = false;
        break;
...
bool POWER::setPowerBoostKeepOn(bool en) {
  uint8_t data;
  if (T4.I2C.readByte(IP5306_ADDR, IP5306_REG_SYS_CTL0, &data) == true) {
    return T4.I2C.writeByte(IP5306_ADDR, IP5306_REG_SYS_CTL0, en ? (data | BOOST_OUT_BIT)  : (data & (~BOOST_OUT_BIT)));
  }
  return false;
}
...
bool POWER::setAutoBootOnLoad(bool en) {
  uint8_t data;
  if (T4.I2C.readByte(IP5306_ADDR, IP5306_REG_SYS_CTL0, &data) == true) {
    return T4.I2C.writeByte(IP5306_ADDR, IP5306_REG_SYS_CTL0, en ? (data | BOOT_ON_LOAD_BIT) : (data & (~BOOT_ON_LOAD_BIT)));
  }
  return false;
}
...
bool POWER::setCharge(bool en) {
  uint8_t data;
  if (T4.I2C.readByte(IP5306_ADDR, IP5306_REG_SYS_CTL0, &data) == true) {
      return T4.I2C.writeByte(IP5306_ADDR, IP5306_REG_SYS_CTL0, en ? (data | CHARGE_OUT_BIT) : (data & (~CHARGE_OUT_BIT)));
  }
  return false;
}
...
bool POWER::isChargeFull() {
  uint8_t data;
  return (T4.I2C.readByte(IP5306_ADDR, IP5306_REG_READ1, &data))
...
bool POWER::canControl() {
  return T4.I2C.writeCommand(IP5306_ADDR, IP5306_REG_READ0);
}

//true:charge, false:discharge
bool POWER::isCharging() {
  uint8_t data;
  return (T4.I2C.readByte(IP5306_ADDR, IP5306_REG_READ0, &data))
...
int8_t POWER::getBatteryLevel() {
  uint8_t data;
  if (T4.I2C.readByte(IP5306_ADDR, IP5306_REG_READ3, &data) == true) {
    switch (data & 0xF0) {
      case 0x00:
        return 100;
      case 0x80:
...
void POWER::powerOFF(){
  ...
  //Power off request
  if (T4.I2C.readByte(IP5306_ADDR, IP5306_REG_SYS_CTL1, &data) == true){
    T4.I2C.writeByte(IP5306_ADDR, IP5306_REG_SYS_CTL1, (data & (~BOOST_ENABLE_BIT)));
  }
